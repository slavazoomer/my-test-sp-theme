...
class CollectionSelector extends HTMLElement {
  constructor() {
    ...
    this.selectorElms = Array.from(this.querySelectorAll('input'));
    for (const elm of this.selectorElms) {
  ...
  toggleCollection(selectedId) {
    document.querySelectorAll(`[data-container-type="collection"][data-section-id="${this.sectionId}"]`).forEach(elm => {
      elm.style.display = "none";
    });
    const selectedCollection = document.querySelector(`[data-container-type="collection"][data-section-id="${this.sectionId}"][data-collection-id="${selectedId}"]`);
    if (selectedCollection === null) {
      throw `collection element not found: %{selectedId}`;
    }
    selectedCollection.style.display = "initial";
  }
...
// If the user clicks on the star rating, scroll to the Yotpo widget
document.addEventListener('page:loaded', function() {
  const starRatingElm = document.querySelector('#star-rating');
  const yotpoWidget = document.querySelector('.yotpo-widget-instance');
  if ((!starRatingElm) || (!yotpoWidget)) {
    return;
  }
  starRatingElm.style.cursor = 'pointer';
  starRatingElm.addEventListener('click', function() {
    yotpoWidget.scrollIntoView({behavior: 'smooth', block: 'start'});
  });
});

// Randomized holiday icons
(() => {
  const holidayIcons = {
    gifts:
      "https://cdn.shopify.com/s/files/1/0123/4314/8608/files/philosophy-gifts.png?v=1603812323",
    candycanes:
      "https://cdn.shopify.com/s/files/1/0123/4314/8608/files/candycanes.png?v=1603817251",
    ornaments:
      "https://cdn.shopify.com/s/files/1/0123/4314/8608/files/ornaments.png?v=1603814631",
    wind: "https://cdn.shopify.com/s/files/1/0123/4314/8608/files/wind.png?v=1603814630",
    snowman:
      "https://cdn.shopify.com/s/files/1/0123/4314/8608/files/snowman.png?v=1603825798",
    mail: "https://cdn.shopify.com/s/files/1/0123/4314/8608/files/mail.png?v=1603815995",
    mittens:
      "https://cdn.shopify.com/s/files/1/0123/4314/8608/files/mittens.png?v=1603825269",
  };
  document.addEventListener('page:loaded', () => {
    if (!window.enableHolidayIcons) {
      return;
    }
    document.querySelectorAll(".image-with-text, .product__media-wrapper, [data-holiday-icon]").forEach((el) => {
      const randomIcon = holidayIcons[Object.keys(holidayIcons)[Math.floor(Math.random() * Object.keys(holidayIcons).length)]];
      el.style.setProperty("--holiday-icon-url", `url(${randomIcon})`);
    });
  });
})();

// Update rewards on variant change
document.addEventListener('variant:change', function(e) {
  // Explanation: on the PDP, there is a span with class "swell-product-helper" that contains the number of points
  // that customers will earn by buying the product in the loyalty program. At the time this is written, it's 1
  // point per dollar, but that is subject to change. It is set up by Yotpo in a file called app.v1.0.368.js inside
  // a setupDisplayPoints() function. Unfortunately, it doesn't change when the variant changes. In addition, it's not as simple
  // as updating the data-attribute on the span, because that will not update the contents of the span. After the first
  // call to setupDisplayPoints(), the amount is saved, so setting the data attribute has no effect.
  // The Yotpo code looks for the data in a jQuery "data()" call,
  // which will fall back to a data attribute if it hasn't been set yet, but since it has, then updating the data
  // attribute has no effect. In addition, if we were to use the global jQuery object, that wouldn't work either, because
  // jQuery saves this updated "data()" in a way that it is scoped to the jQuery "instance", so we need to use
  // Yotpo's jQuery instance. Finally, we need to trigger the setupDisplayPoints() function, which, thankfully, is available
  // on the globally-scoped "spapi" object.
  const variant = e.detail.variant;
  if (!variant) {
    console.warn("No variant found in variant:change event");
  }
  if (!variant.price) {
    console.warn("No price found in variant: ", variant);
    return;
  }
  const helperElement = spapi.$('.swell-product-helper');
  if (!helperElement) {
    return;
  }
  helperElement.data('price-cents', parseInt(variant.price));
  spapi && spapi.setupDisplayPoints();
});
